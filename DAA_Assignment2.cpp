...
int main() {
    ...
    // convert input to lowercase for better input flexibility
    transform(sort_by.begin(), sort_by.end(), sort_by.begin(), ::tolower);
    ...
    int start_index = max(0, static_cast<int>(movies.size()) - 10);
    for (int i = movies.size() - 1; i >= start_index; --i) {
        movies[i].display();
    }
